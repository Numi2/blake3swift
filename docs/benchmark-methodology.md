...
CPU `read`, `memoryMapped`, and `memoryMappedParallel` file benchmark modes are enabled with `--file-modes read,mmap,mmap-parallel`. `read` timing includes file open/stat, bounded file reads into reusable buffers, overlapped CPU subtree reductions for regular files, digest finalization, and close; unknown-length files fall back to the serial streaming read loop. Regular-file reads use two bounded buffers by default so the next `read(2)` can fill one buffer while the previous tile's BLAKE3 subtree reduction runs on the CPU worker pool. `memoryMapped` includes open/stat, `mmap`, bounded tiled hashing, digest finalization, `munmap`, and close. `memoryMappedParallel` uses a direct parallel tree over the mapped region for files up to the implementation cap, currently 2 GiB, then falls back to the bounded 16 MiB subtree-tiled path above that cap to avoid unbounded chaining-value workspace growth. These modes exclude benchmark file creation and full-file Swift byte-array allocation.
...
The staged Metal read path is the bounded-copy alternative to mapped-file Metal hashing. It opens and stats the file, allocates bounded shared Metal staging buffers sized to the configured tile, enables best-effort Darwin read-ahead for the file descriptor, reads tiles directly into `MTLBuffer.contents()`, hashes complete non-final chunks on the GPU, merges chunk or subtree CVs into the Swift CV stack, and finalizes the last BLAKE3 chunk on the CPU. By default, four staging buffers and a matching preallocated async workspace are used so file reads can overlap multiple in-flight GPU reductions without reusing scratch or chunk-CV output buffers early. The default staged-read tile is 32 MiB; tiled Metal mmap and CPU regular-file reads keep their 64 MiB defaults. The staged-read path uses a higher subtree-decomposition threshold than mapped Metal so the default exact-size final prefix is handled by one chunk-CV write plus CPU stack merge instead of many small final-prefix subtree commands.
...
For file publication work, prefer `benchmarks/run-file-reality.sh` over a single all-file-mode invocation. The script runs each strategy in a separate process, captures thermal snapshots around each mode, validates every JSON report, and can insert cooldown between modes. This keeps file-cache/page-in and run-order effects visible instead of hiding them inside one mixed table.
